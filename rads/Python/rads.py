...
def get_data_in_file(infile_name, contig):
    data = ''
    with open(infile_name, 'r') as f:
        found = False
        for line in f:
            if line[0]=='>':              
                if contig == line[1:line.find(' ')]:
                    found = True 
                elif found:
                    return data
            elif found:
...
def get_contig_list(infile):
    contig_list = []
    with open(infile, 'r') as f:
        for line in f:
                if line[0]=='>':
                    contig_list.append(line[1:line.find(' ')])
    return contig_list
...
if enz_num == 1:
    ...
    Enz1Sites = 0
    contig_list = get_contig_list(infile_name)
    for contig in contig_list:
        print('Processing '+contig)
        longstring=get_data_in_file(infile_name,contig)
        sitelist=[]
        position=0
